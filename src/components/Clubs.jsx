...
export default function Clubs({ session, player }) {
  ...
  function isClubOrganizer(club) {
    return isLeagueAdmin() || club.created_by === player?.id
  }

  function myMembership(clubId) {
    return memberships.find(m => m.club_id === clubId && m.player_id === player?.id)
  }
  ...
  async function handleSaveClub(e) {
    ...
    setShowForm(false); setEditingClub(null)
    fetchData()
    setSaving(false)
  ...
  async function handleDeleteClub(clubId) {
    ...
    await supabase.from('organizations').delete().eq('id', clubId)
  ...
  async function handleApproveMember(membershipId, playerId, clubName) {
    ...
    await supabase.from('players').update({ org: clubName }).eq('id', playerId)
    fetchData()
  }
  ...
  async function handleMakeOrganizer(membershipId) {
    await supabase.from('club_members').update({ role: 'organizer' }).eq('id', membershipId)
    fetchData()
  }

  async function handleRemoveMember(membershipId, playerId) {
    await supabase.from('club_members').delete().eq('id', membershipId)
    await supabase.from('players').update({ org: null }).eq('id', playerId)
    fetchData()
  }
      ...
      <div>
        ...
        <div style={{ background: 'white', border: '0.5px solid #c8cdd6', borderRadius: 12, padding: 24, marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
            ...
            {canManage && (
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                {isLeagueAdmin() && (
                  <button onClick={() => handleDeleteClub(club.id)} style={{ background: 'white', border: '0.5px solid #9f1239', borderRadius: 8, padding: '7px 14px', fontSize: 12, fontFamily: 'sans-serif', color: '#9f1239', cursor: 'pointer' }}>
                    Delete
                  </button>
                )}
              </div>
          ...
          {myMembership(club.id)?.status === 'pending' && (
            <div style={{ background: '#fef3c7', border: '0.5px solid #fcd34d', borderRadius: 8, padding: '10px 14px', fontSize: 12, fontFamily: 'sans-serif', color: '#92400e' }}>
              Your join request is pending approval.
            </div>
          )}
          {myMembership(club.id)?.status === 'approved' && (
            <div style={{ background: '#d1fae5', border: '0.5px solid #6ee7b7', borderRadius: 8, padding: '10px 14px', fontSize: 12, fontFamily: 'sans-serif', color: '#065f46' }}>
                ...
                return (
                  <div key={m.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: '#f9f9f7', borderRadius: 8 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      <div style={{ fontSize: 13, fontWeight: 700, color: '#9f1239', fontFamily: 'Playfair Display, serif' }}>{p?.elo}</div>
    ...
    <div>
      ...
      {clubs.length === 0 ? (
        <div style={{ background: 'white', border: '0.5px dashed #c8cdd6', borderRadius: 12, padding: 40, textAlign: 'center' }}>
          <div style={{ fontSize: 14, color: '#888', fontFamily: 'sans-serif' }}>No clubs yet — add one to get started.</div>
        </div>
      ) : (
              ...
              <div key={club.id} onClick={() => setSelectedClub(club.id)} style={{ background: 'white', border: '0.5px solid #c8cdd6', borderRadius: 10, padding: 16, cursor: 'pointer', position: 'relative' }}>
                ...
                {membership && (
                  <div style={{ marginBottom: 8 }}>
                    <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, fontFamily: 'sans-serif', fontWeight: 600, background: membership.status === 'approved' ? '#d1fae5' : '#fef3c7', color: membership.status === 'approved' ? '#065f46' : '#92400e' }}>
