  ...
  return (
    <div style={{ maxWidth: 420, margin: '0 auto', padding: '40px 16px' }}>
      <div style={{ background: 'white', border: '0.5px solid #c8cdd6', borderRadius: 12, padding: 28 }}>
        ...
        {message && (
          <div style={{ background: '#d1fae5', border: '0.5px solid #6ee7b7', borderRadius: 8, padding: '10px 14px', fontSize: 12, color: '#065f46', fontFamily: "'DM Sans', sans-serif", marginBottom: 16 }}>
            {message}
          </div>
        )}
        ...
        <button
          ...
          style={{ width: '100%', background: 'white', border: '0.5px solid #c8cdd6', borderRadius: 8, padding: '10px', fontSize: 13, fontFamily: "'DM Sans', sans-serif", color: '#0F172A', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 16 }}
        >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          ...
          <span style={{ fontSize: 11, color: '#888', fontFamily: "'DM Sans', sans-serif" }}>or</span>
          <div style={{ flex: 1, height: '0.5px', background: '#c8cdd6' }}/>
        </div>
        ...
        <form onSubmit={mode === 'signin' ? handleSignIn : handleSignUp}>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 11, color: '#888', fontFamily: "'DM Sans', sans-serif", display: 'block', marginBottom: 4 }}>Email address</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="jane@email.com" required />
          </div>
          <div style={{ marginBottom: 20 }}>
            <label style={{ fontSize: 11, color: '#888', fontFamily: "'DM Sans', sans-serif", display: 'block', marginBottom: 4 }}>Password</label>
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width: '100%', background: '#0F172A', color: '#ffffff', border: 'none', borderRadius: 8, padding: '11px', fontSize: 13, fontFamily: "'Outfit', sans-serif", fontWeight: 700 }}
          >
            {loading ? 'Please wait...' : mode === 'signin' ? 'Sign In' : 'Create Account'}
          </button>
        </form>

        <div style={{ textAlign: 'center', marginTop: 16, fontSize: 12, fontFamily: "'DM Sans', sans-serif", color: '#888' }}>
          {mode === 'signin' ? (
            <>Don't have an account?{' '}<span onClick={() => setMode('signup')} style={{ color: '#0F172A', fontWeight: 700, cursor: 'pointer' }}>Sign up</span></>
          ) : (
            <>Already have an account?{' '}<span onClick={() => setMode('signin')} style={{ color: '#0F172A', fontWeight: 700, cursor: 'pointer' }}>Sign in</span></>
          )}
        </div>
      </div>
    </div>
