    ...
    <div style={{ maxWidth: 420, margin: '0 auto', padding: '40px 16px' }}>
      <div style={{ background: 'white', border: '0.5px solid #c8cdd6', borderRadius: 12, padding: 28 }}>
        ...
        <button
          ...
          style={{ width: '100%', background: 'white', border: '0.5px solid #c8cdd6', borderRadius: 8, padding: '10px', fontSize: 13, fontFamily: 'sans-serif', color: '#1e2b65', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 16 }}
        >
        ...
        <form onSubmit={mode === 'signin' ? handleSignIn : handleSignUp}>
          ...
          <button
            type="submit"
            disabled={loading}
            style={{ width: '100%', background: '#1e2b65', color: '#ffffff', border: 'none', borderRadius: 8, padding: '11px', fontSize: 13, fontFamily: 'Playfair Display, serif', fontWeight: 700 }}
          >
        ...
        <div style={{ textAlign: 'center', marginTop: 16, fontSize: 12, fontFamily: 'sans-serif', color: '#888' }}>
          {mode === 'signin' ? (
            <>Don't have an account?{' '}<span onClick={() => setMode('signup')} style={{ color: '#1e2b65', fontWeight: 700, cursor: 'pointer' }}>Sign up</span></>
          ) : (
            <>Already have an account?{' '}<span onClick={() => setMode('signin')} style={{ color: '#1e2b65', fontWeight: 700, cursor: 'pointer' }}>Sign in</span></>
          )}
        </div>
      </div>
    </div>
