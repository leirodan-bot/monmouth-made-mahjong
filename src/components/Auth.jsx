...
import { isNative, browser } from '../native'
...
export default function Auth() {
  ...
  async function handleGoogle() {
    ...
    try {
      if (isNative) {
        // On native: get the OAuth URL, then open Safari.
        // After auth, Supabase redirects to com.mahjrank.app://login-callback
        // which iOS routes back to the app via the custom URL scheme.
        // The deep link listener in App.jsx picks up the tokens.
        const { data, error: oauthError } = await supabase.auth.signInWithOAuth({
          provider: 'google',
          options: {
            redirectTo: 'com.mahjrank.app://login-callback',
            skipBrowserRedirect: true,
          },
        })
        if (oauthError) { setError('OAuth error: ' + oauthError.message); return }
        if (data?.url) {
          // Open in-app browser (SFSafariViewController via @capacitor/browser).
          // This keeps the user inside the app per App Store Guideline 4.0.
          // After auth, Supabase redirects to com.mahjrank.app://login-callback
          // which closes the in-app browser and routes back to the app.
          await browser.open(data.url)
        } else {
  ...
  async function handleApple() {
    try {
      if (isNative) {
        // On native: same pattern as Google — get OAuth URL, open Safari,
        // redirect back via custom URL scheme after Apple auth completes.
        const { data, error: oauthError } = await supabase.auth.signInWithOAuth({
          provider: 'apple',
          options: {
            redirectTo: 'com.mahjrank.app://login-callback',
        ...
        if (oauthError) { setError('Apple sign-in error: ' + oauthError.message); return }
        if (data?.url) {
          await browser.open(data.url)
        } else {
          setError('No Apple OAuth URL returned')
        }
      } else {
        // On web: normal redirect
      ...
      if (err?.message?.includes('cancelled') || err?.message?.includes('1001')) return
      ...
      <div style={{ background: 'white', border: `1px solid ${C.border}`, borderRadius: 14, padding: 28 }}>
        ...
        {isNative && (
          <button onClick={handleApple} style={{
            width: '100%', background: '#000', border: 'none', borderRadius: 8, padding: '10px', fontSize: 13,
            fontFamily: "'DM Sans', sans-serif", color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 16, cursor: 'pointer'
          }}>
