    ...
    if (matches) {
      for (const m of matches) {
        ...
        allItems.push({
          id: `match-${m.id}`,
          type: 'game',
          time: new Date(m.played_at),
          icon: m.is_wall_game ? '🧱' : '🀄',
          title: m.is_wall_game
            ? `${submitter?.name || 'Someone'} recorded a wall game`
            : `${winnerP?.name || 'Unknown'} won a game`,
          detail: [
            m.hand_section ? `${m.hand_section} Line ${m.hand_line}` : null,
            m.hand_points ? `${m.hand_points} pts` : null,
            m.jokerless ? 'Jokerless' : null,
            m.win_method === 'self_pick' ? 'Self-picked' : null,
          ].filter(Boolean).join(' · ') || null,
          status: statusLabel,
          statusColor,
          eloUpdates: m.elo_updates,
    ...
    if (badges) {
      for (const b of badges) {
        ...
        allItems.push({
          ...
          icon: badge.emoji,
          title: `${p.name} earned "${badge.name}"`,
          detail: badge.desc,
        })
      }
    }
        ...
        <div style={{ display: 'grid', gap: 0 }}>
          {filtered.map((item, i) => (
            <div key={item.id} style={{
              ...
              background: 'white',
              borderBottom: i < filtered.length - 1 ? '0.5px solid #e8e8e4' : 'none',
              borderTop: i === 0 ? '0.5px solid #c8cdd6' : 'none',
              borderLeft: '0.5px solid #c8cdd6',
